...
export class ListComponent extends BaseComponent implements OnInit {
  ...
  constructor(deps: BaseDependency, private dialog: MatDialog) { 
    super(deps); // Initialize base class (for shared logic)
  }
  ...
  // Dialog form to add a new subdivision
  onAdd(): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === true) {
        this.loadSubdivision();  // Reload after adding
      }
    });
  }
  ...
  onDelete(element: Subdivision): void {
      Swal.fire({
          ...
          if (result.isConfirmed) {
              ...
              this.adminService.deleteSubdivision(element.id).subscribe(
                  () => {
