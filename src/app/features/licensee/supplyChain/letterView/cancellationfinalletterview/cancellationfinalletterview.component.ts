...
@Component({
  selector: 'app-cancellationfinalletterview',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './cancellationfinalletterview.component.html',
  styleUrl: './cancellationfinalletterview.component.scss'
})
export class CancellationfinalletterviewComponent implements OnInit {
    ...
    if (this.isBrowser) {
      ...
      if (cancellationId) {
        ...
        if (approvedFlag === 'true' && source === 'commissioner-dashboard') {
          console.log('Loading final letter for just-approved cancellation');
          this.loadSampleFinalLetterData(cancellationId);
        } else {
          this.loadCancellationFinalLetterData(cancellationId);
        }
      } else {
        this.errorMessage = 'Cancellation ID not provided';
      }
    }
  }
  ...
  private mapApiDataToFinalLetter(apiData: any): void {
    ...
    const distilleryName = passedDistillery ? decodeURIComponent(passedDistillery) : 
                          (apiData.distilleryName || apiData.distillery_name || apiData.branchName || apiData.branch_name || 'M/s Mount Distilleries Ltd');
    ...
    this.cancellationLetterData = {
      ...
      referenceDate: new Date(apiData.cancellationDate || apiData.cancellation_date || apiData.requisitionDate || apiData.requisition_date || Date.now()),
      distilleryName: distilleryName,
      distilleryAddress: this.generateDistilleryAddress(distilleryName),
      exciseOfficerName: 'The Excise Officer-in-Charge',
      permitNumbers: this.extractPermitNumbersFromReference(referenceNo),
      ...
      status: apiData.status || 'ApprovedCancellationByCommissioner'
    };
  }
  ...
  printFinalLetter(): void {
    ...
    win.document.write(`<!doctype html>
      <html>
        <head>
          <title>Cancellation Final Letter - ${letterNumber}</title>
          <meta charset="utf-8">
